...
def read_rdf(metadata_df, file_count_max = 200):
    ...
    file_count = 0
    for i, row in tqdm(metadata_df.iterrows(), total=len(metadata_df), desc="Building TChain"):
        sub = row['subfolder']
...
def main():
    ...
    # read neutrino MC
    neutrino_df = metadata_vars["MC_neutrino_2024_ve"]    
    neutrino_rdf, neutrino_chain, neutrino_int_lumi = read_rdf(neutrino_df, 2000)
    ...
    if data_rdf is not None:
        ...
        counts = cutflow_counts(data_rdf, cuts)
    ...
    cutflow_path = os.path.join(output_dir, "cutflow_summary_2024.csv")
