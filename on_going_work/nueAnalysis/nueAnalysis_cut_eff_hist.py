...
def extract_info_from_partition(
    args,
    metadata_dir="/afs/cern.ch/user/z/zhibin/work/snd-ml/snakemake/metadata/updated",
    tree_name="sndData",
    input_col="nueAnalysis_path",
    lumi_col="lumi_per_file",
):
...
def process_histogram(args, chain, total_lumi, info):
    ...
    outfile = f"{args.outdir}/{mode}_{partition}.root"

    tree_name = chain.GetName()
    df = ROOT.RDataFrame(chain)

    base_filter = "dataQuality == 1 && avgScifiFiducial == 1 && noVetoHit == 1"

    # Extra neutrino filter only when interaction/flavor are defined
    neutrino_filter = build_neutrino_filter(info)

    if neutrino_filter:
        full_filter = f"({base_filter}) && ({neutrino_filter})"
    else:
        full_filter = base_filter

    df_filtered = df.Filter(full_filter, "FinalSelection")
    ...
    opts.fMode = "RECREATE"
    df_filtered.Snapshot(tree_name, outfile, "", opts)
    ...
    print(f"Efficiency          : {passed_events / total_events:.6f}" if total_events > 0 else "Efficiency          : 0.000000")
...
def process_efficiency(args, chain, total_lumi, info):
    ...
    if info.is_mc:
        cut_exprs = [
            ("avgScifiFiducial", "avgScifiFiducial == 1"),
            # ("USBarsVeto", "USBarsVeto == 1"),
            ("noVetoHit", "noVetoHit == 1"),
            ("consecutiveSciFiHits", "consecutiveSciFiHits == 1"),
            ("SciFiContinuity", "SciFiContinuity == 1"),
            ("USPlaneHit_0_1", "USPlaneHit_0_1 == 1"),
            ("SciFiHit35", "SciFiHit35 == 1"),
            # ("USQDC600", "USQDC600 == 1"),
            ("NoHitLastDS", "NoHitLastDS == 1"),
        ]
    else:
        cut_exprs = [
            ("stableBeam", "stableBeam == 1"),
            ("IP1BunchCrossing", "IP1BunchCrossing == 1"),
            ("PreEvtClockCycle100", "PreEvtClockCycle100 == 1"),
            ("avgScifiFiducial", "avgScifiFiducial == 1"),
            # ("USBarsVeto", "USBarsVeto == 1"),
            ("noVetoHit", "noVetoHit == 1"),
            ("consecutiveSciFiHits", "consecutiveSciFiHits == 1"),
    ...
    for cut_name in tqdm(all_cut_names, desc="Building cutflow"):
        if info.is_mc and cut_name in ["stableBeam", "IP1BunchCrossing", "PreEvtClockCycle100"]:
            count = total_events
        else:
...
def main(args):
    ...
    if args.mode == "eff":
        ...
        raise ValueError(f"Invalid mode: {args.mode}. Must be 'eff' or 'hist'.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m", "--mode",
        dest="mode",
        help="calculate efficiency (eff) or histogram (hist)",
    )
    parser.add_argument(
        "-o", "--outdir",
        dest="outdir",
