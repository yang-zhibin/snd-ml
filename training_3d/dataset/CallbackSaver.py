...
class PklSaver(Callback): # only operate on batch_size == 1
    def __init__(self, pkl_hit_path, model_name, out_path):
        super().__init__()
    ...
    def on_test_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=0):   
        ...
        ids = batch.ids
        pdgCode, runId, eventId = ids[0],ids[1],ids[2]
        data_entry = {
                    "pdgCode": pdgCode.item(),
                    "runId": runId.item(),
                    "eventId": eventId.item(),
                    "hits_prediction": predictions, # [n_hits,1]
                }
    ...
    def on_test_epoch_end(self, trainer, pl_module):
        ...
        compress_level = 5
        with gzip.open(self.out_path, 'wb', compresslevel=compress_level) as f:
            pickle.dump(self.data, f)
            #torch.save(event_data, chunk_out_path)
        print(f"Saved data to {self.out_path}, compress level {compress_level}")
