...
class GravNet_3D(GNNBase):
    def __init__(self, hparams):
        ...
        combine_features_size = self.aggregation_factor*self.n_grav_heads*self.output_size
        #print("combine_features_size",combine_features_size)
        ...
        if (self.hparams['use_event_feature']):
            self.global_feature_encoder = make_mlp(
                self.global_feature_channels,
                 [self.layer_structure[0][0]] * hparams["nb_encoder_layer"]+[combine_features_size],
                hidden_activation=hparams["hidden_activation"],
                output_activation=hparams["output_activation"],
                layer_norm=hparams["layernorm"],
                batch_norm=hparams["batchnorm"],
            )
            combine_features_size *=2
        
        #print("output size",self.output_size)
        #print("layer sturcture",self.layer_structure)
        output_layer_input_size = combine_features_size + self.layer_structure[0][0]
    ...
    def output_step(self, hit_encoded_features,n_hits, all_x, evt_feature, batch):
        ...
        #print("hit_encoded_features",hit_encoded_features.shape)
        #print("combine_features shape", combine_features.shape)

        event_feature_expanded = torch.repeat_interleave(combine_features, n_hits, dim=0)

        hits_combine_features = torch.cat([hit_encoded_features, event_feature_expanded], dim=1)

        return self.output_network(hits_combine_features)

    def forward(self, batch, log_attention=False):
        x = batch.x
        n_hits =  batch.n_hits
        #print()
        #print("x shape",x.shape, "y shape:", batch.y.shape, "n_hits shape:",n_hits.shape)
        #print()
        x = self.batch_norm(x)
        ...
        #print(batch.y.shape)
        # Encode all features
        #print("feature_encoder structure")
        ...
        return self.output_step(encoded_features, n_hits,  all_hidden_features, golbal_encoded_features, batch.batch)
