...
from dataset.torchGeoDataset import PredGeoDataset_3D
from models.GravNet.Models.gravnet_3d import GravNet_3D
from dataset.CallbackSaver import PklSaver
...
def main(args):
    ...
    pkl_hit_path = args.input
    print("pkl file: ",pkl_hit_path)
    ...
    print('reading model...')
    if accelerator == 'cpu':
        model = GravNet_3D.load_from_checkpoint(ckpt_path, map_location=torch.device('cpu'))
    else:
        model = GravNet_3D.load_from_checkpoint(ckpt_path)


    tmp_dir = args.tmpdir
    print("Temporary directory for dataloader:", tmp_dir)
    
    test_data= PredGeoDataset_3D(root=tmp_dir, pkl_file=pkl_hit_path, split='test', use_event_feature=config['use_event_feature'], weight_type=config['weight_type'], force_reload=True)
    ...
    trainer = Trainer(
        ...
        devices="auto",
        callbacks=[PklSaver(pkl_hit_path, model_name, out_path)],
        )
...
#python pred_3d.py -i  /eos/experiment/sndlhc/users/zhibin/MC_neutrino/volTarget_100fb-1/241/pkl_hit_MC_neutrino_volTarget_100fb-1_241.pkl.gz -o /eos/experiment/sndlhc/users/zhibin/MC_neutrino/volTarget_100fb-1/241/model_baseline_3d_output_MC_neutrino_volTarget_100fb-1_241.pkl.gz -t ./test_data 