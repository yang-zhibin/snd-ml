...
def selection_distribution(df, signal, score):
    #muon_df = df.Filter(f'ParticleType=="muon"')
    neutrino_df = df.Filter(f'signal == 1')

    #muon_df.Snapshot("tree", "/afs/cern.ch/user/z/zhibin/work/snd-ml/src/evaluate_v2/selected_data/muon_selection.root")
    neutrino_df.Snapshot("tree", "/afs/cern.ch/user/z/zhibin/work/snd-ml/src/evaluate_v2/selected_data/neutrino_selection.root")
def main(model, signal, score):
    ...
    selection_distribution(df, signal, score)
