...
def compute_matrix_with_expected(df):
    ...
    confusion_matrix.at["muon", "ve"] = None
    confusion_matrix.at["muon", "vm"] = None
    confusion_matrix.at["muon", "vt"] = None
    confusion_matrix.at["muon", "NC"] = None
    print(confusion_matrix)
...
def process(df, signal, score):
    ...
    #confusion_matrix = compute_matrix(df)
    confusion_matrix = compute_matrix_with_expected(df)
...
def main(model, signal, score):
    ...
    for filename in os.listdir(model_output_path):
        ...
        if filename.endswith(".root") and (filename.startswith("test_neutrino") or filename.startswith("test_muon")):
            if 'test_muon_outside' in filename:
                continue
    ...
    confusion_matrix = process(df, signal, score)

    print("RDataFrame run times: ",df.GetNRuns())

    #print(confusion_matrix)
    csv_path = f'/afs/cern.ch/user/z/zhibin/work/snd-ml/src/evaluate_v2/csv_matrix/{model}_scifi_acceptance_confusion_matrix_with_veto.csv'
    confusion_matrix.to_csv(csv_path, index=False)
