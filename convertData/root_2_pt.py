...
def process_column_data(hit_array, veto_hit_array, feature_array, hit_column_name, veto_hit_column_name, feature_column_name, idx):
    ...
    veto_hit_tensors = {
        col.split('.')[-1]: torch.tensor(veto_hit_array[col][idx], dtype=torch.float32) for col in veto_hit_column_name
    }
    
    one_event_data = {
        "hitFeature": hit_tensors,
        "vetoHitFeature": veto_hit_tensors,
    }
...
def main(args):
    ...
    veto_hit_column_name = [
        'vetoHits/vetoHits.detType', 'vetoHits/vetoHits.orientation', 'vetoHits/vetoHits.x1',
        'vetoHits/vetoHits.y1', 'vetoHits/vetoHits.z1', 'vetoHits/vetoHits.x2',
        'vetoHits/vetoHits.y2', 'vetoHits/vetoHits.z2',
        'vetoHits/vetoHits.qdc','vetoHits/vetoHits.hitTime'
    ]
    id_column_name = [
    ...
    with uproot.open(hit_path) as hit_file:
        ...
        for start in range(0, total_events, chunk_size):
            ...
            veto_hit_array = hit_tree.arrays(veto_hit_column_name, entry_start=start, entry_stop=end)
            ...
            for idx in range(num_events_in_chunk):
                one_event_data = process_column_data(hit_array, veto_hit_array, feature_array, 
                                                     hit_column_name, veto_hit_column_name, 
