...
def resolve_metadata_csv(info: PartitionInfo, metadata_dir: Union[str, Path]) -> Path:
...
def get_partition_info(partition: str, metadata_dir: Union[str, Path]) -> PartitionInfo:
...
def build_tchain_and_lumi(
    ...
    for _, row in df.iterrows():
        ...
        cutflow_path = f"{base}/{rel}"
        digi_path = str(row["digi_path"])

        missing_this_row = False

        if not os.path.exists(digi_path):
            print(f"[WARNING] Digi file not found: {digi_path}")
            missing_this_row = True

        if not os.path.exists(cutflow_path):
            print(f"[WARNING] Cutflow file not found: {cutflow_path}")
            missing_this_row = True
        ...
        chain.Add(digi_path)
        cutflow_chain.Add(cutflow_path)

        lumi_val = 0.0
        if lumi_col in df.columns:
            val = row[lumi_col]
            lumi_val = 0.0 if pd.isna(val) else float(val)

        lumi_per_file[digi_path] = lumi_val
        total_lumi += lumi_val
        n_added += 1
    ...
    print(f"\nFiles added to TChain: {n_added}")
    print(f"Skipped rows due to missing files: {n_missing}")
...
def process_digi(args, chain, total_lumi, info):
    os.makedirs(args.outdir, exist_ok=True)

    partition = args.partition
    outfile = f"{args.outdir}/digi_{partition}.root"

    # Alias awkward branch names before building the dataframe
    chain.SetAlias("EventDeltat_m1_100", "EventDeltat_-1_100")

    df = ROOT.RDataFrame(chain)

    neutrino_filter = build_neutrino_filter(info)
    if neutrino_filter:
        # Only keep this if the filter expression expects pdgCode
        df = df.Define("pdgCode", "species")
        df = df.Filter(neutrino_filter, "neutrino_filter")

    if "muon" in args.partition:
        selection = "AvgSFChan == 1 && NoVetoHits == 0"
    else:
        selection = "AvgSFChan == 1 && NoVetoHits == 1"

    is_mc = "mc" in partition.lower()
    if not is_mc:
        selection += " && StableBeams == 1 && IP1 == 1 && EventDeltat_m1_100 == 1"

    df = df.Filter(selection, "baseline_selection")
    
    # save df to the output file  
    df.Snapshot("sndData", outfile)
    f = ROOT.TFile(outfile, "UPDATE")

    lumi = ROOT.TParameter("double")("total_lumi", float(total_lumi))
    lumi.Write()
...
def main(args):
    ...
    process_digi(args, chain, total_lumi, info)
