...
do  
    ...
    TARGET_DIR="${BASE_OUT_DIR}/${i_run}"
    converted_file=$(find "$TARGET_DIR" -type f -name "*converted*")

    

    if [ -n "$converted_file" ]; then
        echo "found converted file"    
        xrdcp -f ${TARGET_DIR}/* ./${tmp_dir}/
        ...
        xrdcp -f ./${tmp_dir}/${base_name}_converted_00${i_run}.root ${BASE_OUT_DIR}/${i_run}/ 
        rm -r ${tmp_dir}
